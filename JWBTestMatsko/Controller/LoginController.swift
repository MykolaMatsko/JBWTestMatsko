...
class LoginController: UIViewController {
    ...
    let nameTextField: UITextField = {
        let tf = UITextField()
        tf.placeholder = "Name"
        tf.attributedPlaceholder = NSAttributedString(string: tf.placeholder!, attributes: [NSAttributedStringKey.foregroundColor : UIColor.red])
    ...
    func handleLogin() {
        ...
        guard let url = URL(string: "http://apiecho.cf/api/login/") else { return }
                    ...
                    if let data = data {
                        do {
                            if let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String : Any] {
                                print(json)
                                let fetchData = json["data"] as? [String : Any]
                                if let fetchToken = fetchData!["access_token"] as? String {
                                    print("Token:", fetchToken)
                                    self.delegateLog?.accessTokenStored(value: fetchToken)
                                }
                            }
                        } catch {
                            print(error)
...
//        dismiss(animated: true, completion: nil)
    ...
    func handleRegister() {
        ...
        guard let url = URL(string: "http://apiecho.cf/api/signup/") else { return }
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.addValue("application/json", forHTTPHeaderField: "Content-Type")
